...
import { Sparkles, MessageSquare, Settings, BarChart3, DollarSign, MoreHorizontal, Wallet, LayoutDashboard, BellRing, Lightbulb, Cable, Users, CalendarDays } from 'lucide-react';
...
export default function TabNavigation({ activeTab, onTabChange }) {
  ...
  const NavButton = ({ tab, active, compact = false, onClick }) => {
    const Icon = tab.icon;
    return (
      <button
        onClick={onClick}
        className={`w-full flex items-center gap-3 rounded-2xl transition-all ${compact ? 'flex-col justify-center min-h-[72px] px-1 py-2 text-center' : 'justify-between px-4 py-3.5'} ${active ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-800'}`}
      >
        <div className={`flex items-center gap-3 ${compact ? 'flex-col gap-1.5' : ''}`}>
          <div className={`flex items-center justify-center rounded-2xl ${compact ? 'w-11 h-11' : 'w-10 h-10'} ${active ? 'bg-white/15' : 'bg-gray-100 dark:bg-slate-800'}`}>
  ...
  return (
    <>
      <aside className="hidden md:flex fixed top-0 right-0 h-screen w-[19rem] z-50 border-l border-gray-200 dark:border-slate-800 bg-white/96 dark:bg-slate-950/96 backdrop-blur-xl flex-col p-4">
        <div className="flex items-center gap-3 rounded-3xl border border-gray-200 dark:border-slate-800 p-3 mb-5 bg-slate-50/80 dark:bg-slate-900/80">
          <div className="w-12 h-12 rounded-2xl overflow-hidden flex items-center justify-center bg-gradient-to-br from-indigo-100 to-purple-100 dark:from-indigo-900/30 dark:to-purple-900/30">
        ...
        <div className="space-y-2">
          ...
          {mainTabs.map((tab) => <NavButton key={tab.id} tab={tab} active={activeTab === tab.id} onClick={() => onTabChange(tab.id)} />)}
        </div>

        <div className="space-y-2 mt-5">
          <div className="px-2 text-xs font-semibold text-gray-500 dark:text-gray-400">הגדרות ומערכת</div>
          {secondaryTabs.map((tab) => <NavButton key={tab.id} tab={tab} active={activeTab === tab.id} onClick={() => onTabChange(tab.id)} />)}
        </div>

        <div className="mt-auto pt-4">
          <Button variant="outline" className="w-full justify-between rounded-2xl min-h-[50px]" onClick={() => window.location.assign('/calendar')}>
