    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="text-center">
          <p className="text-gray-500 mb-4">הכלי לא נמצא</p>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 flex-1">
            {tool.logo ? (
              <img src={tool.logo} alt={tool.name} className="w-12 h-12 rounded-2xl object-cover shadow-lg" />
            ) : (
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">סיכום מיומנות והתקדמות למידה</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <MetricCard
            icon={BookOpen}
            label="התקדמות תוכנית"
            value={`${masteryData.planProgress}%`}
            color="text-blue-600 bg-blue-50 dark:bg-blue-950/30"
          />
          <MetricCard
            icon={CheckCircle2}
            label="משימות הושלמו"
            value={`${masteryData.completedTasks}/${masteryData.totalTasks}`}
            color="text-emerald-600 bg-emerald-50 dark:bg-emerald-950/30"
          />
          <MetricCard
            icon={Award}
            label="דירוג אישי"
            value={`${masteryData.ratingScore}%`}
            color="text-purple-600 bg-purple-50 dark:bg-purple-950/30"
          />
          <MetricCard
            icon={Zap}
            label="אינטנסיביות שימוש"
            value={`${masteryData.usageScore}%`}
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={masteryData.progressHistory}>
                    ...
                    <XAxis dataKey="step" tick={{ fontSize: 12 }} reversed />
              ...
              {masteryData.plan.steps.map((step, i) => (
                <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-slate-800/50">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className={`text-sm font-medium ${step.isCompleted ? 'line-through text-gray-400' : 'text-gray-700 dark:text-gray-200'}`}>
                      {step.title}
                    </div>
                    {step.dueDate && (
                  ...
                  {step.isCompleted && <Badge className="bg-emerald-100 text-emerald-700">הושלם</Badge>}
                </div>
              ))}
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              {tasks.slice(0, 5).map((task) => (
                <div key={task.id} className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-800/50">
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${
                    task.isCompleted ? 'bg-emerald-500 text-white' : 'border-2 border-gray-300 dark:border-slate-600'
                  }`}>
                    {task.isCompleted && <CheckCircle2 className="w-3 h-3" />}
                  </div>
                  <span className={`text-sm flex-1 ${task.isCompleted ? 'line-through text-gray-400' : 'text-gray-700 dark:text-gray-200'}`}>
                    {task.title}
                  </span>
